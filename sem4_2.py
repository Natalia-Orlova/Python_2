...
with open("sem4.txt", "a", encoding="utf-8") as seminar4:
    seminar4.write(f"уравнение: {a}*x^2 + {b}*x + {c}\n")
    if d > 0 and a:
        x1 = (-b + sqrt(d)) / (2*a)
        x2 = (-b - sqrt(d)) / (2*a)
        seminar4.write(f"корни уравнения = {x1}, {x2}\n")
    elif d == 0:
        x = -b / 2*a
        seminar4.write(f"корень уравнения = {x}\n")
    else:
        seminar4.write("корней нет")
